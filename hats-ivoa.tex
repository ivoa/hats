...
If present, it MUST be a CSV (comma-separated-values) file, with the columns \texttt{"Norder"}, \texttt{"Npix"}, \texttt{"join\_Norder"}, and \texttt{"join\_Npix"}, as shown in example contents in Figure~\ref{fig:examplePartitionJoinInfoCsv}.
Additional columns might be present in the file, but are not required, and may not be interpreted by all HATS readers.
...
\end{lstlisting}
\end{minipage}
\caption{Example \texttt{partition\_join\_info.csv} file contents}
\label{fig:examplePartitionJoinInfoCsv}
\end{figure}
...
The text file may contain comment lines, beginning with the \texttt{'\#'} character.
An example \texttt{properties} file is shown in Figure~\ref{fig:exampleCollectionPropertiesFile}.
...
<capability standardID="ivo://ivoa.net/std/hats#hats-1.0">
  <interface role="std" version="1.0" xsi:type="vr:WebBrowser">
...
\textbf{TODO} - Talk about dumping files on some basic apache server.
...
\begin{verbatim}
  http://<server-address>/<catalog>/Norder=<>/Dir=<>/Npix<>?[<extra-GET-arg>&[...]]
\end{verbatim}
  where \texttt{<server-address>} and \texttt{<path>} are URI-compliant components indicating the domain address and local location path where the service is deployed. 
  
  \textbf{TODO} - also talk about the npix, norder, and suffix as path parts.

  Note that when it contains extra GET arguments, the base URL ends in an ampersand, \texttt{\&}; if there are no extra arguments, then it ends in a question mark, ?.
...
If requesting a subset of columns, 

\begin{itemize}
  \item if required, the column \texttt{\_healpix\_29} must be the first column name
  \item you can specified maximum 8 columns (in addition to \texttt{\_healpix\_29})
  \item unusual column names (starting with a number, containing a minus, ...) must be specified using
\end{itemize}

The service is not required to return the column in the same order they were requested in the query parameters.
