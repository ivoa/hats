...
\editor{Melissa DeLucchi, Neven Caplar, François-Xavier Pineau}
...
The  \texttt{catalog} organization structure is shown in Figure~\ref{fig:exampleCatalogStructure}.
...
\end{lstlisting}
\end{minipage}
\caption{Example catalog directory contents}
\label{fig:exampleCatalogStructure}
\end{figure}
...
We see the directory structure in Figure~\ref{fig:datasetWithLeaf}, showing a dataset with leaf Parquet files at several HEALPix orders.
...
\end{lstlisting} 
\end{minipage}
\caption{Example catalog dataset directory contents}
\label{fig:datasetWithLeaf}
\end{figure}
...
Such a directory structure would appear as shown in Figure~\ref{fig:datasetWithDir}.
...
\end{lstlisting} 
\end{minipage} 
\caption{Example catalog dataset directory contents with leaf directories }
\label{fig:datasetWithDir}
\end{figure}
...
\subsubsection{Spatial Index} \label{sec:spatialIndex}

The HATS format RECOMMENDS that the first column of the dataset is a spatial index column.
This column stores crucial spatial information about the position in the sky for each row, and it is not guaranteed to be unique.
...
This MUST be is calculated from the row's right ascension (from the \texttt{hats\_col\_ra} column value) and declination (from the \texttt{hats\_col\_dec} column value).
If two objects occur at the same location (or the data is individual observations of the same sky object), then multiple rows may have the same value for the column.

This document RECOMMENDS 29 as the HEALPix order used in the calculation, as it provides an incredibly granular index (HEALPix order 29 correspond to the mean separation between HEALPix pixels of 0.000393 arcsec), and is the largest such that will fit into a signed 64-bit integer.
The name of the column and order at which it is calculated are specified in the \texttt{hats.properties} file (detailed in Section~\ref{}), as seen in Figure~\ref{fig:healpix29Properties}.

\begin{figure}
\begin{minipage}{\linewidth}
\begin{lstlisting}
hats_col_healpix       = healpix29
hats_col_healpix_order = 29
\end{lstlisting} 
\end{minipage} 
\caption{Example properties for the HEALPix spatial index column}
\label{fig:healpix29Properties}
\end{figure}
...
Instead, we have a flat storage of the parquet files, as shown in Figure~\ref{fig:exampleIndexStructure}.
...
\begin{minipage}{\linewidth}
  ...
  \end{lstlisting}
\end{minipage}
\caption{Example index table directory contents}
\label{fig:exampleIndexStructure}
\end{figure}
...
\end{lstlisting}
\end{minipage}
\caption{Example collection directory contents}
...
HATS implementations utilize auxiliary files and metadata files to store relevant information about the structure.
A HATS catalog may contain other arbitrary files in the catalog root directory level, as desired by the catalog publisher, or necessary for data access.
A HATS reader MUST allow for unexpected files at the catalog root directory level.
...
Spaces between keyword, equal sign, and value are ignored, but can make the file more human-readable.

An example \texttt{hats.properties} file is shown in Figure~\ref{fig:examplePropertiesFile}.
...
\end{lstlisting}
\end{minipage}
\caption{Example \texttt{hats.properties} file contents}
...
{\footnotesize
\begin{longtable}[h!]{p{0.33\textwidth} p{0.67\textwidth}}
\sptablerule
...
hats\_col\_healpix &Column name that provides a spatial index of HEALPix values at some fixed, high order \\
hats\_col\_healpix\_order &For the \texttt{hats\_col\_healpix} column, what is the HEALPix order \\
...
hats\_cols\_sort &Space-delimited list of the columns used to sort the data. If this isn't specified, it is assumed to be sorted by \texttt{hats\_col\_healpix} column (if present). \\
...
hats\_skymap\_order &HEALPix order of the default \texttt{'skymap.fits'} file \\
hats\_skymap\_alt\_orders &space-delimited list of HEALPix orders for additional alternate skymap files. \\
...
HATS clients may find a \texttt{skymap.fits} file that has been generated at HEALPix order 9 to be very large, and more detail than they need.
Down-sampled skymaps can provide a more manageable file size while still offering a useful overview of the data distribution. 
These skymaps can be generated at lower HEALPix orders, allowing for quicker visualizations and analyses without the need for the full-resolution data.

Additional skymaps are named \texttt{skymap.<healpix order>.fits}, within the same root directory of the catalog as the \texttt{skymap.fits} file, and are listed in the \texttt{hats.properties} file.

Example snippets are shown in Figure~\ref{fig:skymapProperties}.
For the example, we calculated the skymap at order 8 during catalog ingestion, and have written this to the \texttt{skymap.fits} file.
Based on the data distribution, we have a highest HEALPix order of data partitions of 6.
The catalog provider has downsampled the skymap to orders 2, 4, and 6. 
We expect this catalog to have 4 skymap files at the root level: the primary and default at order 8, and 3 additional down-sampled files.

\begin{figure}
\begin{subfigure}[h]{0.5\linewidth}
\begin{lstlisting}
hats_skymap_order      = 8
hats_skymap_alt_orders = 2 4 6
hats_order             = 6
\end{lstlisting}
\caption{Example properties for the HEALPix order of skymaps}
\end{subfigure}
\hfill
...
skymap.fits
\end{lstlisting}
\caption{Example skymaps in the root directory of the catalog}
\end{subfigure}
\caption{Example catalog with down-sampled skymaps}
\label{fig:skymapProperties}
\end{figure}
...
The text file may contain comment lines, beginning with the \texttt{'\#'} character.
An example \texttt{collection.properties} file is shown in Figure~\ref{fig:exampleCollectionPropertiesFile}.
...
\end{lstlisting}
\end{minipage}
\caption{Example \texttt{collection.properties} file contents}
\label{fig:exampleCollectionPropertiesFile}
\end{figure}

\subsubsection{Images and static summaries}

Catalog providers may wish to provide human users with footprint previews and data summaries.
All such files are OPTIONAL, but we enumerate some of them here to provide motivation, and a framework for consistent naming.
This is not meant to be exhaustive; remember that a HATS reader MUST allow for unexpected files at the catalog root directory level.

\paragraph{skymap.png} 

A file called \texttt{skymap.png} MAY be provided in the catalog root to offer a preview of the distribution of data on the sphere. 
It should match the distribution of data provided in the \texttt{skymap.fits}.

This MAY also be included in the collection root directory, and should reflect the data of the collection's primary catalog.

No required pixel size or display format is specified.

\paragraph{partition\_info.png} 

A file called \texttt{partition\_info.png} MAY be provided to offer a preview of the relative density of HATS partitions. 
This should show the HEALPix order used for partitioning, across the footprint of the catalog data.

Figure~\ref{fig:order} is one such example of plotting partition info.

No required pixel size or display format is specified.

\paragraph{README}

A file called \texttt{README.md}, \texttt{README}, or \texttt{README.txt} MAY be provided to offer a human-readable summary of the HATS catalog.
This MAY contain information such as sky coverage, embed a \texttt{skymap.png}, information on auxiliary tables (e.g. margins), columns, types, 
provenance, acknowledgments, or more user-level information such as accessing the catalog with common tools.
The internal format and content of this file is intentionally not specified.

This MAY also be included in the collection root directory.

\paragraph{index.html}

A file called \texttt{index.html} MAY be provided to offer an HTML summary of the HATS catalog, displayable through any web browser.
This MAY contain information such as sky coverage, embed a \texttt{skymap.png}, information on auxiliary tables (e.g. margins), columns, types, 
provenance, or more user-level information such as accessing the catalog with common tools. 
The internal format and content of this file is intentionally not specified; it is appropriate that this file be styled to suit the catalog provider, and MAY contain any additional links to help the user access and understand the data.

This MAY also be included in the collection root directory.


\subsubsection{Additional Parquet Files}

There are several ways to re-structure the parquet data in ways that compress or summarize the available data.
All such files are OPTIONAL, but we enumerate some of them here to provide motivation, and a framework for consistent naming.
This is not meant to be exhaustive; remember that a HATS reader MUST allow for unexpected files at the catalog root directory level.

\paragraph{data\_thumbnail.parquet} 
...
\paragraph{\_metadata and \_common\_metadata} 
...
\paragraph{per\_partition\_statistics.parquet} 

This is a subset of the data contained in the \texttt{\_metadata}, and provides basic statistics of all values inside the data partitions.
This file is OPTIONAL and RECOMMENDED for object catalogs.
